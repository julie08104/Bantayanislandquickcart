...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['code'])) {
    ...
    if (!empty($code)) {
        ...
        $stmt = $pdo->prepare("SELECT id, verification_code FROM customers WHERE verification_code = ? AND is_verified = 0");
        $stmt->execute([$code]);
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $customer = $stmt->fetch();
            $update_stmt = $pdo->prepare("UPDATE customers SET is_verified = 1, verification_code = NULL WHERE id = ?");
